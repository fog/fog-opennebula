Shindo.tests('Fog::Compute[:opennebula] | flavors collection', ['opennebula']) do
    ...
    tests('The flavors collection should') do
        test('should be a kind of Fog::Compute::OpenNebula::Flavors') do
            flavors.is_a? Fog::Compute::OpenNebula::Flavors
        end
        ...
        tests('should be able to get models') do
            ...
            tests('by filter').succeeds do
                flavors.get_by_filter({ :name => 'fogtest', :id => flavors.first.id })
            end
        end
    end
end
