    ...
    tests('The flavor model should') do
        ...
        tests('have attributes') do
            ...
            tests('The flavor model should respond to') do
                [:name, :id, :to_label, :to_s, :get_cpu, :get_vcpu, :get_memory, :get_raw,
                 :get_disk, :get_os, :get_graphics, :get_nic, :get_sched_ds_requirements, :get_sched_ds_rank, :get_sched_requirements, :get_sched_rank, :get_context, :get_user_variables].each do |attribute|
                    test(attribute.to_s) { flavor.respond_to? attribute }
                end
            end
            tests('The attributes hash should have key') do
                [:name, :id, :content, :cpu, :vcpu, :memory, :os, :graphics, :context,
                 :user_variables].each do |attribute|
                    test(attribute.to_s) { model_attribute_hash.key? attribute }
                end
            end
        end
        test('be a kind of Fog::Compute::OpenNebula::Flavor') do
            flavor.is_a? Fog::Compute::OpenNebula::Flavor
        end
        ...
        test('raw parsed properly') do
            flavor.get_raw == %(RAW=["DATA"="<cpu match='exact'><model fallback='allow'>core2duo</model></cpu>", "TYPE"="kvm"]\n)
        end
    end
end
