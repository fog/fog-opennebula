            ...
            class Real
                ...
                def template_pool(filter = {})
                    ...
                    templates = templates.map do |t|
                        ...
                        if filter[:name] && filter[:name].is_a?(String) && !filter[:name].empty? && (t.to_hash['VMTEMPLATE']['NAME'] != filter[:name])
                            next
                        end
                        if filter[:uname] && filter[:uname].is_a?(String) && !filter[:uname].empty? && (t.to_hash['VMTEMPLATE']['UNAME'] != filter[:uname])
                            next
                        end
                        if filter[:uid] && filter[:uid].is_a?(String) && !filter[:uid].empty? && (t.to_hash['VMTEMPLATE']['UID'] != filter[:uid])
                            next
                        end
                        ...
                        if nics.is_a? Array
                            nics.each do |n|
                                ...
                                h['NIC'] << interfaces.new(:vnet => vnet,
                    ...
                    raise Fog::Compute::OpenNebula::NotFound,
                          'Flavor/Template not found' if templates.empty?
